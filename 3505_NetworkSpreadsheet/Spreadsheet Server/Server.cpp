...
//dependency graphs for circular dependencies
//dependency graphs corresponding to the unique document IDs
map<int, DependencyGraph> graphs;

// map of stacks for undos and redos
//keys are unique document IDs 
//stacks contain pairs of strings for cell names and cell contents
...
void openSpreadsheet(int clientID, int sock, vector<string>messageTokens){
  ...
  if (d !=filename.end()){
    ...
    if(graphs.find(DocID) == graphs.end()){//if this is a new document 
      DependencyGraph graph; //create a new graph 
      pair<int, DependencyGraph> paired;
      paired = make_pair(DocID, graph);
      graphs.insert(paired); //insert new graph into map

    }

    //set up stack of undos
    if(undos.find(DocID)== undos.end()){//if this is a  new document
      stack<pair<string, string> > Stack;//create a new stack

      pair<int, stack<pair<string, string> > > paired ;
      
      paired = make_pair(DocID, Stack);
      
      undos.insert(paired);//insert new stack into map

    }

    //set up stac of redos 
    if(redos.find(DocID)== redos.end()){//if this is a new document
      stack<pair<string, string> > Stack;//make a new stack

      pair<int, stack<pair<string, string> > > paired ;
      
      paired = make_pair(DocID, Stack);
      
      redos.insert(paired);//insert new stack into map
...
void cell_edit(int sock, vector<string> messageTokens)
{
  ...
  //bool for if the latest edit resulted in a circular dependency
  ...
  if(newContents.at(0) == '='){
	...
	graphs.find(DocID)->second.GetCellsToRecalculate(cellName);
      ...
      //catch circular dependency exception and remove all new dependencies
      //set circularDependency bool equal to true
      catch(int param){
    ...
    // save new edit in stack for undo 
    ...
    //loop through clients sending edit messages 
  ...
  }//end of edit code for formulas 

  else{// if the edit was not a formula 
    ...
    //loop through clients, send edit message
    for(map<int, int>::iterator it = clientIDToDocID.begin();it!=clientIDToDocID.end();it++)
      {
	if (it->second == DocID)
	  {
  ...
  //if a circular dependency occured, send error message
...
//method for handling call for undo  
void undo_edit(int sock, vector<string> messageTokens)
{
 ...
 //loop through clients, send edit messages 
  for(map<int, int>::iterator it = clientIDToDocID.begin();it!=clientIDToDocID.end();it++)
      {
	if (it->second == DocID)
	  {
  ...
  //pop off sent undo from the stack  
...
//method to send the list of previously created files
void showFileList(int sock, vector<string> messageTokens){
  string fileNames="0";
  for (map<string,int>::iterator it = filename.begin(); it != filename.end(); ++it)
    {
